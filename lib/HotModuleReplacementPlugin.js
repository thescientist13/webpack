...
class HotModuleReplacementPlugin {
	constructor(options) {
		...
		this.multiStep = options && options.multiStep;
		this.fullBuildTimeout = options && !isNaN(options.fullBuildTimeout) ? options.fullBuildTimeout : 200;
	}
		...
		compiler.plugin("compilation", function(compilation, params) {
			...
			compilation.mainTemplate.plugin("bootstrap", function(source, chunk, hash) {
				...
				return this.asString([
					...
					.replace(/\/\*foreachInstalledChunks\*\//g, chunk.chunks.length > 0 ? "for(let chunkId in installedChunks)" : "let chunkId = " + JSON.stringify(chunk.id) + ";")
				]);
			});
